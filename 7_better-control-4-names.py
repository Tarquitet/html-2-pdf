...
# --- CLASE UI PARA SCROLL ---
class ScrollableFrame(ttk.Frame):
    def __init__(self, container, *args, **kwargs):
        super().__init__(container, *args, **kwargs)
        canvas = tk.Canvas(self, height=200) # Altura fija para la lista de archivos
...
class ProPDFGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet PDF - Control Total V7.1")
    ...
    def build_ui(self):
        ...
        # TITULO
        ttk.Label(main_frame, text="GENERADOR PDF: CONTROL TOTAL", style="Title.TLabel").pack(pady=5)
        ttk.Label(main_frame, text="Define tipo de archivo individual, separadores y calidad exacta.", foreground="#555").pack(pady=(0, 15))
        ...
        f_name = ttk.Frame(name_frame)
        f_name.pack(fill="x")
        
        ttk.Label(f_name, text="Patrón:").pack(side="left")
        pat_combo = ttk.Combobox(f_name, textvariable=self.naming_pattern, width=35)
        pat_combo['values'] = ["{Type}_{Name}_{Role}", "{Name}_{Type}_{Role}", "CV_{Name}_{Role}", "{Type}_{Role}_{Name}"]
        pat_combo.pack(side="left", padx=5)
        
        ttk.Label(f_name, text="Separador:").pack(side="left", padx=(10, 5))
        sep_combo = ttk.Combobox(f_name, textvariable=self.separator_char, width=5, state="readonly")
        sep_combo['values'] = ["_", "-", " "] 
        sep_combo.pack(side="left")
        
        # Preview dinámico
        self.preview_lbl = ttk.Label(name_frame, text="Ejemplo: ...", foreground="blue", font=("Consolas", 9))
        self.preview_lbl.pack(pady=(10,0), anchor="w")
        
        # Triggers
        self.naming_pattern.trace("w", self.update_preview)
        self.separator_char.trace("w", self.update_preview)
        self.first_name.trace("w", self.update_preview)

        # --- 3. ARCHIVOS (LISTA DETALLADA) ---
        file_frame = ttk.LabelFrame(main_frame, text="3. Archivos y Tipo Individual", padding=10)
        ...
        # --- 4. AJUSTES TÉCNICOS ---
        tech_frame = ttk.LabelFrame(main_frame, text="4. Calidad", padding=10)
        tech_frame.pack(fill="x", pady=5)
        
        # Slider Calidad (CORREGIDO: width va en Label, no en pack)
        q_frame = ttk.Frame(tech_frame)
        q_frame.pack(fill="x", pady=5)
        ttk.Label(q_frame, text="Calidad JPEG:", width=15).pack(side="left") 
        self.sc_qual = ttk.Scale(q_frame, from_=10, to=100, variable=self.quality_var, command=self.update_quality_lbl)
        self.sc_qual.pack(side="left", fill="x", expand=True)
        self.lbl_qual_val = ttk.Label(q_frame, text="80", width=4)
        self.lbl_qual_val.pack(side="left", padx=5)
        
        # Slider Resolución (CORREGIDO: width va en Label, no en pack)
        w_frame = ttk.Frame(tech_frame)
        w_frame.pack(fill="x", pady=5)
        ttk.Label(w_frame, text="Ancho Máx (px):", width=15).pack(side="left")
        self.sc_width = ttk.Scale(w_frame, from_=500, to=3000, variable=self.max_width_var, command=self.update_width_lbl)
        self.sc_width.pack(side="left", fill="x", expand=True)
        self.lbl_width_val = ttk.Label(w_frame, text="1600", width=4)
        self.lbl_width_val.pack(side="left", padx=5)
        
        ttk.Checkbutton(tech_frame, text="Limpiar Metadatos", variable=self.clean_meta_var).pack(anchor="w")
    ...
    def update_preview(self, *args):
        try:
            ex_name = self.construct_filename(
                ...
                self.role_es.get(), self.separator_char.get(), 
    ...
    def sel_files(self):
        ...
        self.status.config(text=f"{len(files)} archivos cargados. Revisa los tipos.")
    ...
    def add_file_row(self, f_path):
        ...
        elif "carta" in lower or "letter" in lower: default_type = "Carta"
    ...
    # --- LÓGICA DE NOMBRADO ---
    def clean_str(self, s, sep):
        s = s.strip()
        clean = re.sub(r'[^a-zA-Z0-9]', sep, s)
        while (sep+sep) in clean:
            clean = clean.replace(sep+sep, sep)
        return clean

    def construct_filename(self, doc_type, lang, first, last, role, sep, pattern):
        c_first = self.clean_str(first, sep)
        c_last = self.clean_str(last, sep)
        c_role = self.clean_str(role, sep)
        ...
        try:
            with sync_playwright() as p:
                ...
                for idx, item in enumerate(self.file_rows):
                    ...
                    selected_type = item["type_var"].get()
                    
                    final_name = self.construct_filename(
                        selected_type, lang_code, first, last, curr_role, sep, pat
                    )
                    
                    self.status.config(text=f"Generando: {final_name}...")
