...
# --- UI SCROLLABLE (Optimizada) ---
class ScrollableFrame(ttk.Frame):
    def __init__(self, container, *args, **kwargs):
        super().__init__(container, *args, **kwargs)
        canvas = tk.Canvas(self, height=150, highlightthickness=0) # Altura reducida
...
class ProPDFGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet PDF - Compact V9")
        self.root.geometry("600x720") # Mucho más pequeño y manejable
    ...
    def build_ui(self):
        ...
        ttk.Label(header, text="GENERADOR PDF: SMART NAMING", style="Title.TLabel").pack(side="left")
        ttk.Label(header, text="v9 Compact", style="Small.TLabel").pack(side="right")

        # --- 1. IDENTIDAD (GRID COMPACTO) ---
        info_frame = ttk.LabelFrame(main_frame, text="1. Identidad", padding=(10, 5))
        info_frame.pack(fill="x", pady=2)
        
        # Usamos Grid para aprovechar el ancho
        ...
        # Fila 2: Opciones extra y preview
        ...
        self.lbl_qual_val = ttk.Label(h1, text="80", font=("Arial", 8, "bold")); self.lbl_qual_val.pack(side="right")
        ttk.Scale(c1, from_=10, to=100, variable=self.quality_var, command=self.update_quality_lbl).pack(fill="x")
        ...
        self.lbl_width_val = ttk.Label(h2, text="1600", font=("Arial", 8, "bold")); self.lbl_width_val.pack(side="right")
        ttk.Scale(c2, from_=500, to=3000, variable=self.max_width_var, command=self.update_width_lbl).pack(fill="x")
    ...
    def add_file_row(self, f_path):
        ...
        row.pack(fill="x", pady=1, padx=2) # Padding reducido
        ...
        # Nombre archivo (recortado si es muy largo)
        ...
        cb = ttk.Combobox(row, textvariable=type_var, width=12) # Ancho reducido
        ...
        try:
            with sync_playwright() as p:
                ...
                for idx, item in enumerate(self.file_rows):
                    ...
                    self.status.config(text=f"Creando: {final_name}")
