...
class PortfolioGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet PDF Generator (Ultimate)")
        self.root.geometry("600x650")
        self.html_files = []
        self.port = 0
        self.quality_var = tk.IntVar(value=75) # Valor por defecto
    ...
    def build_ui(self):
        ...
        ttk.Label(frame, text="Versión Servidor Local + Control de Calidad", font=("Arial", 10)).pack(pady=5)
        ...
        # 3. CONTROL DE CALIDAD (NUEVO)
        ttk.Label(frame, text="3. Compresión y Calidad", style="Bold.TLabel").pack(anchor="w", pady=(15, 5))
    ...
    def process_batch(self):
        ...
        # Capturamos la calidad seleccionada por el usuario
        ...
        try:
            with sync_playwright() as p:
                ...
                for index, html_path in enumerate(self.html_files):
                    ...
                    self.status.config(text=f"Comprimiendo ({current_quality}%)...")
                    # Pasamos la calidad seleccionada a la función de guardado
            ...
            messagebox.showinfo("Éxito", f"Se han generado {len(self.html_files)} archivos en:\n{self.out_dir}")
            # NOTA: Se eliminó os.startfile() para que no abra el explorador
            ...
            for img in page_obj.get_images():
                ...
                try:
                    ...
                    with Image.open(io.BytesIO(img_data)) as pil_img:
                        ...
                        # Si la calidad es baja (<50), reducimos también la resolución
                        max_w = 1500
                        ...
                        buffer = io.BytesIO()
                        # Usamos el nivel de calidad del slider
