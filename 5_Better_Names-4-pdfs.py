#!/usr/bin/env python3
"""
Version 5: HTML -> PDF generator with language-aware naming.

This is a versioned copy (5_...) of the original `0_HTML-2-PDF-Python.py`.
It preserves existing behavior and adds detection of `-en` (or `_en`) suffix
in the HTML filename to produce English-labeled PDF output names.
"""
import sys
import os
import subprocess
...
class PortfolioGenerator:
    def __init__(self, root):
        self.root = root
        self.root.title("Tarquitet PDF Generator (Server Mode) v5")
    ...
    def build_ui(self):
        ...
        ttk.Label(frame, text="GENERADOR DE DOSSIER & CV (v5)", font=("Arial", 14, "bold")).pack(pady=10)
    ...
    def get_smart_name(self, original_path):
        ...
        # Detect language suffix -en or _en
        is_en = lower.endswith('-en') or lower.endswith('_en')
        if is_en:
            # remove trailing -en/_en for type detection
            base = re.sub(r'[_-]?en$', '', lower)
        else:
            base = lower

        # Labels mapping: keep Spanish names for Spanish pages, English for -en pages
        spanish_labels = {'artistico': 'Artistico', 'harvard': 'Ver_Harvard'}
        english_labels = {'artistico': 'Artistic', 'harvard': 'Harvard_Ver'}

        if "portfolio" in base or "dossier" in base or "index" in base:
            name = "David_Pinto_Portfolio"
            if is_en:
                name = "David_Pinto_Portfolio_EN"
            return f"{name}.pdf"

        elif "cv" in base:
            # detect type keyword after cv (e.g. cv-artistico, cv-harvard)
            parts = re.sub(r'^cv[_-]?', '', base).replace('-', ' ').replace('_', ' ').strip().split()
            cv_key = parts[0] if parts else 'general'
            if is_en:
                label = english_labels.get(cv_key, cv_key.title())
            else:
                label = spanish_labels.get(cv_key, cv_key.title())
            return f"CV_David_Pinto_{label}.pdf"

        else:
            display = base.replace('-', '_').replace(' ', '_')
            if is_en:
                display = f"{display}_EN"
            return f"{display.title()}.pdf"
